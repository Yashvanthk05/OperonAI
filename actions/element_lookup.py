"""Element lookup strategies: by index, by name, by uiautomation, and Excel helpers."""
...
def find_by_index(
    ...
    """Find an interactive element by its numeric index."""
...
def find_by_name(
    state: DesktopState, name: str, control_type: str | None = None
) -> Optional[Tuple[UIElement, BoundingBox]]:
    """Find an interactive element by name (substring match) in the pywinauto tree."""
...
def find_by_uiautomation(name: str) -> Optional[Tuple[int, int]]:
    """Use the uiautomation package to search for a control by name.

    Returns the center (x, y) of the first matching control, or None.
    """
    try:
...
def is_excel_reference(text: str) -> bool:
    """Check if text looks like an Excel cell reference (e.g. A1, B2:C10)."""
